...
/** Returns the data points contained within this series.
 ...
 Data is provided to the chart from its `SChartDatasource`.  The chart uses this data to build up its series, and to populate the `SChartDataSeries` for each one.  When it is populated, each data series makes internal copies of the data objects which are provided to it from the chart datasource.  These internal representations of the data are actually instances of an internal subclass of `SChartDataPoint`, but should be treated as instances of `SChartDataPoint`.  The data series maintains an array of these internal data points.
 
 This property allows you to access the internal data points which the data series holds.  You can use these to display the value of a particular data point, for example in the chart crosshair.  
